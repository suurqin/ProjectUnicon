...
public class XRecyclerView extends RecyclerView {
    ...
    private void init(Context context) {
        ...
        if (pullRefreshEnabled) {
            BaseRefreshHeader refreshHeader = RefreshHeaderFactory.getHeader(context, refreshStyle);
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof BaseRefreshHeader)) {
            BaseRefreshHeader refreshHeader = RefreshHeaderFactory.getHeader(mContext, refreshStyle);
            mHeaderViews.add(0, refreshHeader);
            mRefreshHeader = refreshHeader;
            mRefreshHeader.setProgressStyle(mRefreshProgressStyle);
        }
     ...
     * 加载更多时错误处理
     */
    public void loadMoreErrorComplete() {
        isLoadingData = false;
        View footView = mFootViews.get(0);
        if (footView instanceof LoadingMoreFooter) {
            ((LoadingMoreFooter) footView).setState(LoadingMoreFooter.STATE_COMPLETE);
